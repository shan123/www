...
$heading_format =& $workbook->addFormat(array('Size' => 10,
										...
										'color' => 'black',
										'Border' => 1));
$heading_format->setFontFamily('Arial');

$regular_format =& $workbook->addFormat(array('Size' => 10,
										'Align' => 'left',
										'Border' => 1));
$regular_format -> setFontFamily('Arial');
$nonusableip_format =& $workbook->addFormat(array('Size' => 10,
										...
										'FgColor' => 22,
										'Border' => 1));
$nonusableip_format->setFontFamily('Arial');
...
foreach($tabV as $a => $b) {
	...
	$worksheet->write(3, 4, $sub, $regular_format);
	$worksheet->write(4, 3, 'Segment', $heading_format);
	$worksheet->write(4, 4, $tab, $regular_format);
	$worksheet->write(6, 0, 'Network #', $heading_format);
	$worksheet->write(6, 1, 'IP', $heading_format);
	$worksheet->write(6, 2, 'Own', $heading_format);
	$worksheet->write(6, 3, 'Category', $heading_format);
	$worksheet->write(6, 4, 'Assignment', $heading_format);
	$worksheet->write(6, 5, 'Typical', $heading_format);
	...
	for($j=$node; $j < $bnode-1; $j++) {
		$node = $node + 1;
		$worksheet->write(7+$i, 0, $network, $regular_format);
